...
def run_role_eda(dataset_name, history_path, actions_path, training_path, output_dir, *, only_2026=False):
    ...
    print_section("R4", "현재 전체 분포 기준 Flex Champion")
    min_role_games, min_role_rate = load_current_flex_policy()
    current_stats = current_flex_statistics(actual_counts, min_role_games, min_role_rate)
    current_flex = current_stats.loc[current_stats["current_is_flex"]].sort_values(
        "current_total_games", ascending=False, kind="stable"
    )
    print(f"Current Flex 기준: Role별 {min_role_games}회 이상 및 {min_role_rate:.2%} 이상인 Role이 2개 이상")
    print("Current Flex는 현재 Dataset 전체 완료 경기의 final_position 분포 기준이며, 현재 Champion Role 상태 확인용 EDA입니다.")
    print("정상 Position 기록을 분모로 사용합니다. 과거 시점 예측용 Feature로 사용하거나 processed dataset에 저장하지 않습니다.")
    print(f"Current Flex Champion 수: {len(current_flex)}")
    current_table = pd.DataFrame({"Picks": current_flex["current_total_games"]})
    for role in POSITIONS:
        current_table[f"{role.upper()}%"] = current_flex[f"current_{role}_rate"] * 100
    current_table["Possible Roles"] = current_flex["current_possible_roles"].map(json.dumps)
    print_table(current_table.rename_axis("Champion"))
